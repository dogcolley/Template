		...
		*/?>
        ...
        <tr>
            ...
            <td>
				...
				<p style="margin-top: 10px;font-size: 14px;color: #e40000;">
					공지의 경우 내용만 노출이되고 예약은 정상 가능합니다.
				</p>
            </td>
        </tr>
        <?php } ?>
		...
		<tr>
            <th scope="row"><label for="wr_2">적용예약선택<strong class="sound_only">필수</strong></label></th>
            <td>
				<?if(!$board['bo_1']){?>
				 <span style="color:red">목록을 불러올 게시판을 설정해주세요!</span>
				<?}else{
					//임시기능이라.. 모듈화 없이 바로 넣는습니다.
				?>
				<select name="wr_2" id="wr_2" class="frm_input">
					<option value="">전체</option>
				<?
					$sql = "select * from ".$g5['write_prefix'].$board['bo_1']." where wr_7 != '1' ";
					$qry = sql_query($sql);
					for($i=0; $row=sql_fetch_array($qry); $i++){
						$arr[] = $row;
				?>
					<option value="<?=$row['wr_id']?>" <?if($row['wr_id'] == $wr_2)echo 'selected' ?>><?=$row['wr_subject'].' '.$row['wr_6']?></option>
				<?}?>
				</select>
				<?}?>
				<p style="margin-top:5px">전체휴관이 아닌 특정상품,문의,아이템등의 휴일을 설정할경우에 해당부분을 설정해주세요.</p>
			</td>
        </tr>


        <tr >
            <th scope="row"><label for="wr_content">안내내용<strong class="sound_only">필수</strong></label></th>
            <td class="wr_content">
				<p style="margin-bottom: 10px;font-size: 14px;color: #e40000;">
					미입력시 노출되 지않습니다.
				</p>
